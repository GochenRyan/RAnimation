...
namespace utils
{
    ...
    bool LoadTextureFromMemory(const std::string& name,
                               const uint8_t* data,
                               int dataSize,
                               Texture& texture,
    ...
    struct Material
    {
        ...
        inline bool IsAlphaOpaque() const { return alphaMode == AlphaMode::PREMULTIPLIED; }

        inline bool IsTransparent() const { return alphaMode == AlphaMode::TRANSPARENT; }
        ...
        inline bool IsEmissive() const
        {
            return emissiveTexIndex != StaticTexture::Black &&
                   (emissiveAndRoughnessScale.x != 0.0f || emissiveAndRoughnessScale.y != 0.0f ||
    ...
    struct Instance
    {
        ...
        bool allowUpdate =
                false; // if "false" will be merged into the monolithic BLAS together with other static geometry
    };
